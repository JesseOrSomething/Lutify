document.addEventListener('DOMContentLoaded', () => {
    ...
    const webGLHelper = {
        ...
        init(canvas) {
            ...
            const fragShader = this.createShader(gl.FRAGMENT_SHADER, `precision highp float; varying vec2 v_texCoord; uniform sampler2D u_image; uniform sampler3D u_lut; void main() { vec4 originalColor = texture2D(u_image, v_texCoord); gl_FragColor = texture3D(u_lut, originalColor.rgb);}`);
        ...
        createShader(type, source) { const shader = this.gl.createShader(type); this.gl.shaderSource(shader, source); this.gl.compileShader(shader); return shader; },
        ...
        createTexture(image) {
            const gl = this.gl;
            if (this.imageTexture) gl.deleteTexture(this.imageTexture); // Clean up old texture
        ...
        createIdentityLut() {
            const size = 2; const data = []; for (let z = 0; z < size; z++) for (let y = 0; y < size; y++) for (let x = 0; x < size; x++) data.push(x / (size - 1), y / (size - 1), z / (size - 1));
            const gl = this.gl;
    ...
    let db; let images = []; let luts = []; let selectedImageName = null; let deferredPrompt;

    function init() { if (localStorage.getItem('hasVisitedApp')) showApp(); else showLandingPage(); webGLHelper.init(canvas); initDB(); setupEventListeners(); }
    ...
    function initDB() { const req = indexedDB.open('PixelPerfectDB_v2', 1); req.onupgradeneeded = e => { if (!e.target.result.objectStoreNames.contains('luts')) e.target.result.createObjectStore('luts', { keyPath: 'name' }); }; req.onsuccess = e => { db = e.target.result; loadLutsFromDB(); }; }
    function setupEventListeners() { installButton.addEventListener('click', () => { if (deferredPrompt) deferredPrompt.prompt(); }); enterAppButton.addEventListener('click', showApp); imageUpload.addEventListener('change', handleImageUpload); lutUpload.addEventListener('change', handleLutUpload); photoSelect.addEventListener('change', (e) => selectImage(e.target.value)); lutSelect.addEventListener('change', (e) => selectLut(e.target.value)); }

    function handleImageUpload(e) {
        if (e.target.files.length > 0) { placeholder.style.display = 'none'; canvas.style.display = 'block'; }
        for (const file of e.target.files) {
            if (images.some(img => img.name === file.name)) continue; // Prevent duplicates
            const reader = new FileReader();
            reader.onload = (event) => {
                const img = new Image();
                img.onload = () => {
                    const imageData = { name: file.name, element: img };
                    images.push(imageData); addPhotoToSelect(imageData);
                    if (!selectedImageName) selectImage(imageData.name);
                };
    ...
    function addPhotoToSelect(imageData) { const opt = document.createElement('option'); opt.value = imageData.name; opt.textContent = imageData.name; photoSelect.appendChild(opt); }
